...
void Client::connectToServer()
{
    ...
    if (connectionSocket == INVALID_SOCKET) 
    {
        throw std::runtime_error("[Client] Failed to create socket: " + std::to_string(WSAGetLastError()));
    }
    ...
    {
        throw std::runtime_error("[Client] Invalid address / Address not supported");
    }

    if (connect(connectionSocket, (sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) 
    {
        throw std::runtime_error("[Client] Failed to connect to server: " + std::to_string(WSAGetLastError()));
    }
}
...
void Client::sendMessage(uint8_t commandId, const char* message, uint32_t messageLength) 
{
    ...
    {
        throw std::runtime_error("[Client] Failed to send message header");
    }
    ...
    if (messageLength > 0 && message != nullptr) 
    {
        ...
        {
            throw std::runtime_error("[Client] Failed to send message payload");
        }
    }
}
...
{
    if (connectionSocket == INVALID_SOCKET) throw std::runtime_error("[Client] Not connected to server");
    ...
    {
        if (matrix[i].size() != matrixSize) throw std::runtime_error("[Client] Matrix inner size mismatch");
...
uint8_t Client::processStatus() 
{
    ...
    if (!receiveExact((char*)&header, sizeof(MessageHeader))) 
        throw std::runtime_error("[Client] Connection closed by server while waiting for status header");
    
    uint32_t length = ntohl(header.messageLength);
    if (header.commandId != CMD_CHECK_STATUS || length != 1) 
    {
        throw std::runtime_error("[Client] Invalid response from server for checkStatus");
    }

    uint8_t status;
    if (!receiveExact((char*)&status, 1)) 
        throw std::runtime_error("[Client] Connection closed while reading status byte");
...
std::vector<std::vector<int>> Client::processResult(uint32_t matrixSize) 
{
    ...
    {
        throw std::runtime_error("[Client] Expected GET_RESULT command but got something else");
    }
    ...
    {
        throw std::runtime_error("[Client] Result matrix size mismatch");
    }

    std::vector<int> vectorizedMatrix(matrixSize * matrixSize);
    if (!receiveExact((char*)vectorizedMatrix.data(), expectedBytes)) 
    {
        throw std::runtime_error("[Client] Connection closed while reading result data");
    }
    ...
    for (uint32_t i = 0; i < matrixSize; ++i) 
    {
        for (uint32_t j = 0; j < matrixSize; ++j) 
        {
            result[i][j] = ntohl(vectorizedMatrix[i * matrixSize + j]);
        }
    }
    return result;
}